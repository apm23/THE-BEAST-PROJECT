...
def main() -> int:
    ...
    checks = {
        ...
        "r1_r2_candidate": repo / "local_build" / "REMAKE_PROVEN45_R1_R2" / "data2_payload.pak",
        "r1_r2_manifest": repo / "local_build" / "REMAKE_PROVEN45_R1_R2" / "BUILD_MANIFEST.json",
    ...
    if not checks["compatibility_report"].exists() or not checks["current_runtime_report"].exists():
        ...
        next_action = "Build remaining special-infected/outfit/inventory-stack stages, then combine into final single-player core candidate."
        state = "PARTIAL_CANDIDATE_ONLY"
    else:
        next_action = "Package candidate and execute config/remake_singleplayer_test_matrix.json in game."
